...
with open("birth_info.csv", "r") as in_file, open("birth2.csv", "w") as out_file:
    ...
    for row in x:
       ...
       row.append(data['geonamesbirth'][0]['lng'])
       row.append(data['geonamesbirth'][0]['lat'])
       row.append(data['geonamesbirth'][0]['geonameId'])
       w.writerow(row)
       print(row)
# http://api.geonames.org/searchJSON?q=warsaw%20poland&maxRows=1&username=joshuadull

with open("death_info.csv", "r") as in_file, open("death2.csv", "w") as out_file:
    x = csv.reader(in_file)
    w = csv.writer(out_file)
    row0 = next(x)
    row0.append('longitude')
    ...
    for row in x:
       ...
       print(r.url)	
       data = json.loads(r.text)
       row.append(data['geonamesdeath'][0]['lng'])
       row.append(data['geonamesdeath'][0]['lat'])
       row.append(data['geonamesdeath'][0]['geonameId'])
       w.writerow(row)
       print(row)